<?php

use CodeIgniter\Router\RouteCollection;

/**
 * @var RouteCollection $routes
 */

// ── Auth ──────────────────────────────────────────────────
$routes->get('/',       'Auth::index');
$routes->get('/login',  'Auth::index');
$routes->post('/login', 'Auth::login');
$routes->get('/logout', 'Auth::logout');

// ── Dashboard ─────────────────────────────────────────────
$routes->get('/dashboard', 'Dashboard::index');

// ── Data Dokter ───────────────────────────────────────────
$routes->get('/dokter',            'Dokter::index');
$routes->get('/dokter/tambah',     'Dokter::tambah');
$routes->post('/dokter/simpan',    'Dokter::simpan');
$routes->get('/dokter/edit/(:num)', 'Dokter::edit/$1');
$routes->post('/dokter/update/(:num)', 'Dokter::update/$1');
$routes->get('/dokter/hapus/(:num)', 'Dokter::hapus/$1');

// ── Data Pasien ───────────────────────────────────────────
$routes->get('/pasien',            'Pasien::index');
$routes->get('/pasien/tambah',     'Pasien::tambah');
$routes->post('/pasien/simpan',    'Pasien::simpan');
$routes->get('/pasien/edit/(:num)', 'Pasien::edit/$1');
$routes->post('/pasien/update/(:num)', 'Pasien::update/$1');
$routes->get('/pasien/hapus/(:num)', 'Pasien::hapus/$1');

// ── Rekam Medis ───────────────────────────────────────────
$routes->get('/rekam_medis',              'RekamMedis::index');
$routes->get('/rekam_medis/tambah',       'RekamMedis::tambah');
$routes->post('/rekam_medis/simpan',      'RekamMedis::simpan');
$routes->get('/rekam_medis/detail/(:num)', 'RekamMedis::detail/$1');
$routes->get('/rekam_medis/edit/(:num)',   'RekamMedis::edit/$1');
$routes->post('/rekam_medis/update/(:num)', 'RekamMedis::update/$1');
$routes->get('/rekam_medis/hapus/(:num)', 'RekamMedis::hapus/$1');

// ── Resep ─────────────────────────────────────────────────
$routes->get('/resep',                     'Resep::index');
$routes->get('/resep/tambah/(:num)',        'Resep::tambah/$1'); // (:num) = id_rekam_medis
$routes->post('/resep/simpan',             'Resep::simpan');
$routes->get('/resep/detail/(:num)',        'Resep::detail/$1');
$routes->post('/resep/update_status/(:num)', 'Resep::updateStatus/$1');

// ── Data Obat ─────────────────────────────────────────────
$routes->get('/obat',            'Obat::index');
$routes->get('/obat/tambah',     'Obat::tambah');
$routes->post('/obat/simpan',    'Obat::simpan');
$routes->get('/obat/edit/(:num)', 'Obat::edit/$1');
$routes->post('/obat/update/(:num)', 'Obat::update/$1');
$routes->get('/obat/hapus/(:num)', 'Obat::hapus/$1');
